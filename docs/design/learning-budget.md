...
| root × Stem × Specification derivational grid (Ithkuil-sourced; marks `CN:SPEC` {basic, content, form, object} per root) | GZ: +~1-3h [H] — one 4-way rule applied to every root, derived forms recognised by rule; RZ: n/a (inherited derivation) | ×3-4 assignable meanings per scarce root body with no phonology change — attacks the 22-body monosyllable bottleneck; open: where the marker lives (coda / derivational syllable / script-only) and composition with gf-grammar O/A/P/R classes | **PROPOSED** (ithkuil-forks.md §4; bead conlang-czq) |
