...
script or other things I was going for — all of the grab-low-hanging-
fruit & max-out-useful-traits variety." Answer below, with the
partial systems included as the fifth column. Sources: SPEC +
fusion study + capacity ledger (greenfield); rz-* docs (RZ);
atlantic-zonal + az-latinate-twins (AZ); english-plus-channels +
modes-in-english (Track E); partial-systems.md.
...
## 4. Strategy implied (duke recommendation)

Build order by (value × adoption probability) / effort:

1. **Readback layer + modes-in-English card** (done in draft) — the
   wedge that teaches 40% of greenfield modes incidentally.
2. **RZ cloze pilot** — the measurement that upgrades RZ from design
   to evidence, and prices AZ-b's niche at the same time.
3. **Greenfield continues as the ceiling product** on its own clock —
   fused script + beauty pass + seed lexicon (kps can start:
   the ledger's 22 safe bodies + hook heuristic are ready) — with no
   adoption pressure; its role is to be *right*, and to feed traits
   downhill to the other columns (as modes already did).
4. AZ-b only if the cloze matrix says AZ-a's Romance gist is weak.
