# GZ→RZ mining audit — the priced menu (conlang-faj)

Program lane B→A, sequenced after the scenario packet
(rz-bootstrap-scenarios.md) whose weights apply: reader-facing
quality HIGH, measured-claims machinery HIGH, production tooling
MEDIUM-HIGH, modes/repair MEDIUM-LOW, display script LOW. Evidence
discipline (program.md) applies: ledger hours are [H] until the C1
calibration; pure-upside verdicts are agent-callable (Edward's
gate-calibration ruling, 2026-08-22); everything else lands as
adopt-pending-evidence or declined-with-reasons. The gate:
(1) priced, (2) survives RZ's naturalistic surface, (3) zero
first-contact tax.
...
The audit's shape confirms the ledger's rule of thumb: everything
cohort-flat and lexicon-free was already bought or is bought here
for single-digit hours; every DECLINE is exactly the class that
touches inherited word shapes. The mine's remaining value for RZ
is concentrated in (a) per-mode completions (bcq), (b) the repair
register when S4 unparks, (c) measurement instruments — not in
further grammar. Lane A (RZ excellence) is therefore mostly
*quality* work now, not feature work.
